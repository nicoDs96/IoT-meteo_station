...
class Callback:
    '''
    A class to add a MQTTSN callback. It is a modification of Callback class into lib/ to support forwrding to AWS.
    ...
    def __init__(self):
        self.events = []
        self.registered = {}
        # We need two connection otherwise it become an aggregating gateway
        self.aws_conn_1 = init_mqtt_connection(clientId="station1")
        self.aws_conn_2 = init_mqtt_connection(clientId="station2")
        self.aws_conn_1.connect()
        self.aws_conn_2.connect()
    ...
    def messageArrived(self, topicName, payload, qos, retained, msgid):
        ...
        #get the station that send the message
        from_topic = str(payload)[len('{"station_id":')+1:len('{"station_id":')+7:]
        #from_topic = str(payload)[len('{"station_id":'):len('{"station_id":')+6:]
        
        to_topic = "<ND>"
        #translate the topic to AWS topic
        if from_topic == "stat_1":
            to_topic="station"
            send_data(self.aws_conn_1, payload, to_topic)
        elif from_topic == "stat_2":
            to_topic="station2"
            send_data(self.aws_conn_2, payload, to_topic)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mqttstopics', type=str, default='stat_1 stat_2',
