...
/*Take as input reference to the variables where to store measurements, returns 1 on failure*/
...
static int start_station(char id[], char ipv6[], char port[])
{
...
{
    if( argc <= 3 ){
        printf("Usage: %s station_id broker_ipv6 broker_port_nr\n", argv[0]);
        return 1;
    }
    start_station(argv[1], argv[2],argv[3]);
    return 0;
}
...
static const shell_command_t shell_commands[] = {
        {"read_sensors","Read sensors without sending their values", cmd_read_sensors},
